...
const TaskTable = (props) => {
  ...
  useEffect(() => {
    setMemTasks(
      selectedMem
        ? props.tasks.filter((task) => task.member_id === selectedMem)
        : setMemTasks(
            props.tasks.filter(
              (task) => task.member_id === props.members.reverse()[0]._id
            )
          )
    );
    console.log(memTasks, selectedMem, props.members, mem);
  }, [props.tasks, selectedMem]);
